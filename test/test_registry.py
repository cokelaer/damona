from damona.registry import SingleRegistry, Registry
import glob




def test_singe_registry():
    ...
    sr = SingleRegistry(salmon_registry)
    assert sr.data['releases']['1.3.0']['binaries'] == ['salmon']


    # binaries is provided explicitly in the general section
    fastqc_registry = [x for x in _registry_files if "/fastqc/" in x][0]
    sr = SingleRegistry(fastqc_registry)
    assert sr.data['releases']['0.11.9']['binaries'] == ['fastqc']
    ...
    sr = SingleRegistry(kraken_registry)
    assert sr.data['releases']['1.1']['binaries'] == ['kraken']
    assert sr.data['releases']['2.0.9']['binaries'] == ['kraken2', 'kraken2-build']
